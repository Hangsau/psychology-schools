...
- **P6 品質穩定化試點**：已啟動，P5 暫停。`claims-schema.json`、`verify-claims.py`、strict 結構模式與 `inventory-high-risk.py` 已完成；CBT／精神分析／本土心理學首批共登錄 14 條高風險主張（8 corroborated、6 retrieved）。句級高召回盤點共列出 308 個候選單位，只有 13 個句子／條目已有 anchor 覆蓋，295 個仍待人工裁決；三篇均維持 `in_progress`。
